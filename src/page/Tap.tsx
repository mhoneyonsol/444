...
const Tap = () => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (cnt > 10) {
        ...
        if (waterFall.length > 5) {
          const tmpFall = waterFall.filter((_fall, i) => i !== 3);
          setWaterFall(tmpFall);
        }
        cnt = 0;
      }
      setWaterFall((prev) => {
        const tmp = prev.filter((fall, i) => fall.x > -70 || i === 0);
        return [
          ...tmp.slice(0, 1),
  ...
  const checkCollision = () => {
    if (!spaceRef.current || !coins.length) return false;
  ...
  const checkCrashWithEarth = () => {
    if (!spaceRef.current) return false;
    const spaceshipRect = spaceRef.current.getBoundingClientRect();
  ...
  return (
    <div
      className="w-full h-full flex flex-col overflow-hidden px-2"
      onTouchStart={handleJump} // Handle touch start
    >
      <ToastContainer />
      ...
      <div className='text-center mt-[8%] text-black font-press-start z-10'>
        <div className='text-[84px]' style={{ WebkitTextStrokeColor: 'white', WebkitTextStrokeWidth: 5 }}>
          {achievedCoin}
        </div>
      </div>
      <div>
      ...
      <div
        ...
        onTouchStart={() => { handleStart(); setStarted(true); }}
      >
        <img src={isGoing ? rotatedSpaceUp : rotatedSpaceDown} alt='space-ship' width={100} height={61} />
      </div>
      <div className='flex'>
